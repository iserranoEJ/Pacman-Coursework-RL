...
class MDPAgent(Agent):
    ...
    def isEmpty(self, loc, nearGhosts):
        if loc not in self.food and not loc in self.capsules and not loc in self.ghosts and not loc in nearGhosts:
            return True
        return False
    
    def isFood(self, loc, nearGhosts):
        if loc in self.food and not loc in self.capsules and not loc in self.ghosts and not loc in nearGhosts:
            return True
        return False

    def isGhost(self, loc):
        if loc in self.ghosts:
            return True
        return False

    def isCapsule(self, loc, nearGhosts):
        if loc in self.capsules and not loc in self.ghosts and not loc in nearGhosts and not loc in self.food:
            return True
        return False

    def isNearGhost(self, loc, nearGhosts):
        if loc in nearGhosts:
            return True
        return False
    ...
    def squareNeighbours(self, location, dist):
        locX = location[0]
        locY = location[1]
        neighbours = []
        
        for i in range (locX-dist, locX+dist+1):
            for j in range(locY-dist, locY+dist+1):
